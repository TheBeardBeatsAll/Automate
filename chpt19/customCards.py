...
def make_cards(guestList):
    ...
    os.makedirs('chpt19/imageCards', exist_ok=True)

    # load flower image
    flowerImg = Image.open('chpt19/customCardsFiles/flower.png')
    ...
    with open(guestList) as file:
        for line in file:
            ...
            card_font = ImageFont.truetype('chpt19/customCardsFiles/Pacifico.ttf', 24)
            draw_obj.text((120, 100), guest, fill='red', font=card_font)

            # save resulting image
            imageName = '{}_card.png'.format(guest)
            border.save(os.path.join('chpt19/imageCards', imageName))
    

if __name__ == "__main__":
    make_cards('automate_online-materials/guests.txt')